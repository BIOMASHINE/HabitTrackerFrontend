...
async function getStats() {
    ...
    for (let i = 0; i < habits.length; i++) {
        try {
            const response = await fetch(`https://habittracker-production-api.up.railway.app:8080/api/v1/habits/stats/${habits[i].id}`, {
                headers: {
                    'accept': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('access_token')}`
                }
...
async function loadHabits() {
    try {
        const responseHabits = await fetch('https://habittracker-production-api.up.railway.app:8080/api/v1/habits', {
            headers: {
                'accept': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('access_token')}`
            }
...
habitsList.onclick = async function (event) {
    if (event.target.dataset.index) {
        ...
        if (type === 'submit') {
            ...
            try {
                if (!habit.completed) {
                    const response = await fetch(`https://habittracker-production-api.up.railway.app:8080/api/v1/habits/completions/${habit.id}`, {
                        method: 'POST',
                        headers: {
                            'accept': 'application/json',
                            'Authorization': `Bearer ${localStorage.getItem('access_token')}`
                    ...
                    const statsResponse = await fetch(`https://habittracker-production-api.up.railway.app:8080/api/v1/habits/stats/${habit.id}`, {
                    ...
                    const response = await fetch(`https://habittracker-production-api.up.railway.app:8080/api/v1/habits/completions/${habit.completionId}`, {
                        method: 'DELETE',
                        headers: { 'Authorization': `Bearer ${localStorage.getItem('access_token')}` }
                    });
                    if (response.status === 401) {
            ...
            try {
                const response = await fetch(`https://habittracker-production-api.up.railway.app:8080/api/v1/habits/${habits[index].id}`, {
                    method: 'DELETE',
                    headers: {
                        'accept': 'application/json',
                        'Authorization': `Bearer ${localStorage.getItem('access_token')}`
            ...
            try {
                const response = await fetch(
                    `https://habittracker-production-api.up.railway.app:8080/api/v1/habits/stats/${habits[index].id}?limit=${period}`,
                    {
                        headers: {
                            'accept': 'application/json',
                            'Authorization': `Bearer ${localStorage.getItem('access_token')}`
